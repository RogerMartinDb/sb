...
// external provider feeds (Sportradar etc.), normalises to canonical format,
...
type NormalisedMarketEvent struct {
	EventType   string `json:"event_type"` // "market.update", "market.resulted", etc.
...
// (Sportradar, Betfair Exchange, etc.).
...
// Normaliser transforms a RawProviderEvent into a NormalisedMarketEvent.
...
func (s *IngestionService) consumeFeed(ctx context.Context, feed ProviderFeed) error {
	...
	for raw := range ch {
		event, err := s.normaliser.Normalise(raw)
		if err != nil {
			s.logger.Warn("ingestion: normalise failed", "provider", raw.ProviderID, "err", err)
			continue
		}
		if err := s.publish(event); err != nil {
			s.logger.Error("ingestion: publish failed", "event_type", event.EventType, "err", err)
		}
	}
	return nil
}
...
func (s *IngestionService) publish(event *NormalisedMarketEvent) error {
	...
	// Partition by market_id so all updates for the same market land on the
	// same partition, preserving ordering.
	_, _, err = s.producer.SendMessage(&sarama.ProducerMessage{
		Topic: "market-data.normalised",
		Key:   sarama.StringEncoder(event.MarketID),
